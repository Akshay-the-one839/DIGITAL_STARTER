...
void handle_star_delta_delay() {
   ...
   while (true) {
      ...
      if (sd_delay != last_value || increment_mode != last_mode) {
         ...
         lcd_goto(1, 1);   
         if (increment_mode == 1) {
            printf(lcd_out, "5.STAR DELT NRM");
         }
         else {
            printf(lcd_out, "5.STAR DELT FST");
         }
...
void handle_voltage_setting(void) {
   ...
   while(1) {
      ...
      if(sub_sel != last_sub_sel) {
         lcd_cmd(LCD_CLEAR);
         lcd_goto(1, 1);
         printf(lcd_out, "6.VOLT SETTING  ");
         lcd_goto(1, 2);
         printf(lcd_out, "%s", volt_submenu[sub_sel]);
      ...
      if(input(ENCODER_SW) == 0) {
         ...
         if(input(ENCODER_SW) == 0) {
            ...
            if(sub_sel == 0) {
               ...
               lcd_cmd(LCD_CLEAR);
               lcd_goto(1, 1);
               printf(lcd_out, "LV SAVED        ");
               lcd_goto(1, 2);
               printf(lcd_out, "%lu:%lu:%lu      ", (long)lv_r, (long)lv_y, (long)lv_b);

               // Wait for user to press SW ? back to sub menu
               while(input(ENCODER_SW) == 1);
               delay_ms(20);
               while(input(ENCODER_SW) == 0);
               delay_ms(50);

               last_sub_sel = 255; // force redraw sub menu
            }
            ...
            // --------------------------------
            // OPTION 3  EXIT -> NEXT MENU
            // --------------------------------
            else if(sub_sel == 3) {
               lcd_cmd(LCD_CLEAR);
               lcd_goto(1, 1);
               printf(lcd_out, "VOLT MENU DONE  ");
               lcd_goto(1, 2);
               printf(lcd_out, "Going to next...");
               delay_ms(1000);
               lcd_cmd(LCD_CLEAR);
               advance_to_next_menu();
               return;
...
void handle_dryrun_delay() {
   ...
   while (1) {
      ...
      if (dryrun_delay != last_value || increment_mode != last_mode) {
         ...
         lcd_goto(1, 1);   
         if (increment_mode == 1) {
            printf(lcd_out, "7.DRYRUN DLY NORM");
         }
         else if (increment_mode == 10) {
            printf(lcd_out, "7.DRYRUN DLY FAST");
         }
         else {
            printf(lcd_out, "7.DRYRUN DLY SUFA");
         }
...
void handle_dryrun_restart() {
   ...
   while (1) {
      ...
      if (restart_delay != last_value || increment_mode != last_mode) {
         ...
         lcd_goto(1, 1); 
         if (increment_mode == 1) {
            printf(lcd_out, "8.DRYRUN RST NORM");
         }
         else if (increment_mode == 10) {
            printf(lcd_out, "8.DRYRUN RST FAST");
         }
         else {
            printf(lcd_out, "8.DRYRUN RST SUFA");
         }
...
void handle_dryrun_trip() {
   ...
   while (1) {
      ...
      if (trip_current != last_value || increment_mode != last_mode) {
         ...
         lcd_goto(1, 1); 
         if (increment_mode == 1) {
            printf(lcd_out, "9.DRYRUN TRP NRM");
         }
         else {
            printf(lcd_out, "9.DRYRUN TRP FST");
         }
...
void handle_set_on_time() {
   ...
   while (1) {
      ...
      if (on_time != last_value || increment_mode != last_mode) {
         ...
         lcd_goto(1, 1);
         if (increment_mode == 1) {
            printf(lcd_out, "4.SET ON TIME NRM");
         }
         else if (increment_mode == 10) {
            printf(lcd_out, "4.SET ON TIME FST");
         }
         else {
            printf(lcd_out, "4.SET ON TIME SUF");
         }
